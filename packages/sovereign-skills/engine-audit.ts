...
// Usage: bun run sovereign-skills/engine-audit.ts
...
const SESSION_DIR = join(TAU_DIR, ".tau/agent/sessions/-projects-sovereign-projects-tau/2026-09-08T06-07-13-107Z_01a07fa0-b2d3-76df-9cd1-b3c185165228");
...
function runDiff(): string {
  const cmd = `diff -rq "${VENDOR_OH_MY_PI}" "${ENGINE_PACKAGES}"`;
...
function parseDiff(output: string): DiffRow[] {
  ...
  for (const line of lines) {
    if (line.includes("Only in ")) {
      const parts = line.split(": ");
      if (parts.length === 2) {
        const loc = parts[0].replace("Only in ", "");
        const file = parts[1];
        records.push({
      ...
      const rest = line.replace("diff -r ", "");
      const [vendor, engine] = rest.split(" ");
      records.push({
        type: "modified",
        location: "",
        file: "",
        vendor_path: vendor.replace(`${ENGINE_DIR}/`, ""),
        engine_path: engine.replace(`${ENGINE_DIR}/`, ""),
      });
    }
  }
  return records;
...
function saveCSV(df: DiffRow[], path: string) {
  ...
  const rows = df
    .map(
      (r) =>
        `${r.type},"${r.location}","${r.file}","${r.vendor_path || ""}","${r.engine_path || ""}"`
    )
    .join("\n");
...
function main() {
  ...
  const csvPath = join(SESSION_DIR, "engine-vendor-diff.csv");
  const mdPath = join(SESSION_DIR, "engine-audit.md");
