---
id: sidechat-watch-madeon
title: 'Side-chat watch: Madeon tickets'
enabled: true
mode: task
concurrency:
  max_running: 1
...
---
Side-chat watchdog for "Lane-4 Madeon side chat" — deterministic thin worker.
...
Step 1 — cached aid:
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --print-aid
...
a. Resolve the aid. One query, no OR-joins, no broadening:
   SELECT agent_id FROM agent.agents WHERE session_id='cc3ea0e2-2fb8-45f3-846c-b86ef71210ac' AND kind='root' ORDER BY created_at DESC LIMIT 1
b. Seed the watermark WITHOUT scanning the transcript:
   ...
   messages):
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --seed <aid> -1
...
c. On success:
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --seed <aid> <last_seq>

Step 3 — the read (local transcript primary, DB fallback):
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --read-jsonl > /tmp/scw-madeon-rows.json
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --query
...
rm -f /tmp/scw-madeon-rows.json and write the result as a NEW file (creation is
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id madeon --rows /tmp/scw-madeon-rows.json
