...
---
Side-chat watchdog for "Find Madeon tickets for Mission Ballroom" (chat_id: cc3ea0e2-2fb8-45f3-846c-b86ef71210ac).
...
1. Resolve the chat's root agent (a side chat's chat_id equals its agent's session_id):
   ...
   WHERE session_id = 'cc3ea0e2-2fb8-45f3-846c-b86ef71210ac' AND kind = 'root'
...
2. Read the agent's latest turns (bounded, index-backed). Do NOT put a
   ...
   turns exist. Stay silent (no user notification) and write a one-line final
   summary anyway (e.g. "madeon watch <timestamp>: 0 recent turns, nothing to
   report"). Do not retry, broaden, or run additional queries — an empty result
   is the answer, not an error.
2b. Timeout/error path (repaired 2026-09-18 after 69 worker timeouts): if ANY
   DB query returns an error — including a statement timeout — write the error
   text into your final message and STOP immediately. Do NOT retry the query,
   do NOT broaden it, do NOT run alternative or "check" queries. One attempt
   per query; a failed query is a failed run, reported honestly, never a
...
4. If every recent user message already has a substantive reply: stay silent
   (no user notification) but ALWAYS write the one-line final summary from
   step 2. A 'succeeded' run with an empty summary is indistinguishable from a
   crashed run — never leave the summary empty.
