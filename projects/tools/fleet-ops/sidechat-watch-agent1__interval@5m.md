...
---
Side-chat watchdog for "Lane-4 Agent-1 side chat" — deterministic thin worker.
...
Step 1 — cached aid:
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --print-aid
...
a. Resolve the aid. One query, no OR-joins, no broadening:
   SELECT agent_id FROM agent.agents WHERE session_id='8a756bd0-3361-427f-8b30-2ec0860e91e5' AND kind='root' ORDER BY created_at DESC LIMIT 1
b. Seed the watermark WITHOUT scanning the transcript:
   ...
   messages):
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --seed <aid> -1
...
c. On success:
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --seed <aid> <last_seq>

Step 3 — the read (local transcript primary, DB fallback):
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --read-jsonl > /tmp/scw-agent1-rows.json
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --query
...
rm -f /tmp/scw-agent1-rows.json and write the result as a NEW file (creation is
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent1 --rows /tmp/scw-agent1-rows.json
