...
- If zero rows: stay silent. End the turn with no chat message. Silence is the signal.
...
- FINAL MESSAGE: always write a one-line result summary, even when silent in chat. Format: "spawn-error-watch <ts>: N infra-signature hits in 10m window" (N=0 is a complete result). A succeeded run with an empty summary is indistinguishable from a crashed run — never leave the summary empty.
