---
id: sidechat-watch-agent2
title: 'Side-chat watch: Agent 2'
enabled: true
mode: task
concurrency:
  max_running: 1
...
---
Side-chat watchdog for "Lane-4 Agent-2 side chat" — deterministic thin worker.
...
Step 1 — cached aid:
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --print-aid
...
a. Resolve the aid. One query, no OR-joins, no broadening:
   SELECT agent_id FROM agent.agents WHERE session_id='0fcb5f23-25d7-44de-9a7d-76342c7b4dd8' AND kind='root' ORDER BY created_at DESC LIMIT 1
b. Seed the watermark WITHOUT scanning the transcript:
   ...
   messages):
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --seed <aid> -1
...
c. On success:
   ~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --seed <aid> <last_seq>

Step 3 — the read (local transcript primary, DB fallback):
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --read-jsonl > /tmp/scw-agent2-rows.json
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --query
...
rm -f /tmp/scw-agent2-rows.json and write the result as a NEW file (creation is
...
~/workspace/bin/taskhook run -- python3 ~/workspace/bin/sidechat-watch.py --watch-id agent2 --rows /tmp/scw-agent2-rows.json
