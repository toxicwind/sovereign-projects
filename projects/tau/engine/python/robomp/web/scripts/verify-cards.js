...
async function resolveChrome(explicit) {
    ...
    candidates.push("/usr/bin/google-chrome-stable", "/usr/bin/google-chrome", "/usr/bin/chromium", "/usr/bin/chromium-browser", "/snap/bin/chromium", "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome");
...
async function main() {
    ...
    let statusMock = baseStatus;
    let statusDelayMs = 0;
    let customIndexHtml = null;
    ...
    page.on("request", async (req) => {
        ...
        if (url.pathname === "/api/status") {
            ...
            const parsedBody = req.postData() ? JSON.parse(req.postData()) : null;
    ...
    const terminalLeakKeys = cards
        .filter((card) => card.text.includes("superseded-del") ||
    ...
    const debugTextContrast = await page.evaluate(() => {
        ...
        const getBgColor = (el, fallback = "lab(15 0 0)") => {
            let cur = el;
            while (cur) {
    ...
    html = html.replace(/<script id="robomp-config" type="application\/json">[^<]*<\/script>/, '<script id="robomp-config" type="application/json">{"replayEnabled":false,"replayToken":""}</script>');
    ...
    results.push({
        ...
        ok: readonly.chipText.includes("trigger disabled · read-only") &&
    ...
    const responsiveOk = await page.evaluate(() => {
        ...
        const noPageOverflow = docEl.scrollWidth <= docEl.clientWidth + 1 &&
    ...
    const replayHtml = (await replayResp.text()).replace(/<script id="robomp-config" type="application\/json">[^<]*<\/script>/, `<script id="robomp-config" type="application/json">{"replayEnabled":true,"replayToken":"${REPLAY_TOKEN}"}</script>`);
    ...
    await page.evaluate(() => {
        const btn = document.querySelector(".rmp-card[data-bucket='failed'] button");
        btn?.click();
    });
    await Bun.sleep(100); // wait for request and state update
    const retryReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/trigger") && r.body?.mode === "retry" && r.body?.delivery_id === "fail-del-1");
    ...
    await page.evaluate(() => {
        ...
        const btn = card?.querySelector("button.danger");
        btn?.click();
    });
    await Bun.sleep(100);
    const cancelReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/cancel") && r.body?.delivery_id === "run-del-2");
    ...
    await Bun.sleep(100);
    const triageReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/trigger") && r.body?.mode === "triage" && r.body?.issue === "octo/widget#5");
    results.push({
    ...
    await Bun.sleep(100);
    const retryLatestReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/trigger") && r.body?.mode === "retry" && r.body?.issue === "octo/widget#5");
    results.push({
    ...
    await Bun.sleep(100);
    const enterReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/trigger") && r.body?.mode === "triage" && r.body?.issue === "octo/widget#5");
    results.push({
    ...
    await page.evaluate(() => {
        const navBtn = Array.from(document.querySelectorAll(".rmp-nav-item")).find((b) => b.querySelector(".rmp-nav-item-label")?.textContent?.trim() === "Activity");
        navBtn?.click();
    });
    ...
    await page.evaluate(() => {
        const btn = Array.from(document.querySelectorAll("table.t button")).find((b) => b.textContent?.trim() === "retry");
        btn?.click();
    });
    ...
    const activityRetryReqs = triggeredRequests.filter((r) => r.url.endsWith("/api/trigger") && r.body?.mode === "retry");
