...
const CANCEL_CONFIRM = "Kill this running task? The omp subprocess dies and the row lands in 'failed'.";
...
export function IssueCard(props) {
    ...
    const ref = () => item().ref;
    const full = () => ref() !== null;
    return (<article class="rmp-card" data-bucket={item().bucket}>
      <Show when={full()} fallback={<CompactHead item={item()}/>}>
        ...
        <LifecycleStepper state={item().issueState} classification={item().classification} failed={item().bucket === "failed"} live={item().live !== null || item().inflightOnly}/>
      </Show>
      <MetaRow item={item()}/>
      ...
      <Show when={CONFIG.replayEnabled &&
        ...
        <div class="rmp-card-actions">
          <Show when={item().bucket === "failed" && item().deliveryId}>
            <button class="tiny" onClick={() => void runTrigger({ mode: "retry", delivery_id: item().deliveryId })}>
              retry
            </button>
          </Show>
...
function CompactHead(props) {
    return (<div class="rmp-card-head">
      <div class="rmp-card-id">
...
function ActivityPill(props) {
    const state = () => props.item.latestEvent?.state ?? (props.item.bucket === "running" ? "running" : undefined);
    const label = () => props.item.latestEvent?.state ??
        (props.item.bucket === "running" ? (props.item.inflightOnly ? "inflight" : "running") : "—");
    return (<Pill state={state()} dot={props.item.bucket === "running"}>
      {label()}
    </Pill>);
}
function MetaRow(props) {
    ...
    return (<div class="rmp-card-meta">
      <Show when={live()}>
...
async function cancel(deliveryId) {
    if (!window.confirm(CANCEL_CONFIRM))
        return;
