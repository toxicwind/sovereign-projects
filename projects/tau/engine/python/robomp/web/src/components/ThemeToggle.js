import { preference, setPreference, } from "../theme";
...
export function ThemeToggle() {
    ...
    const label = () => `${PREFERENCE_LABEL[preference()]} — click to switch`;
    return (<button type="button" class="btn ghost tiny" onClick={() => setPreference(NEXT_PREFERENCE[preference()])} aria-label={label()} title={label()}>
      {icon()}
    </button>);
}
