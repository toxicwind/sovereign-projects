...
export function Releases() {
    const releases = () => statusResource()?.releases ?? [];
    return (<GlassCard heading="releases" accessory={<span class="tabular">{releases().length}</span>}>
...
function ReleaseTableRow(props) {
    const releaseUrl = () => `https://github.com/${props.release.repo}/releases/tag/${encodeURIComponent(props.release.tag)}`;
    return (<tr>
      <td>
        <a class="font-mono text-[12px] text-ink-100 hover:text-accent-2" href={releaseUrl()} target="_blank" rel="noopener">
          {props.release.key}
        </a>
      </td>
      <td>
