...
import { TERMINAL_ISSUE_STATES, } from "./types";
export const CODE_STAGES = ["new", "reproducing", "fixing", "PR", "done"];
export const SIMPLE_STAGES = ["triaged", "resolved"];
export const SIMPLE_CLASSIFICATIONS = new Set([
    "question",
    "enhancement",
    "proposal",
    "invalid",
...
export function stageOrdinal(state) {
    return state ? STATE_ORDINAL[state] ?? 0 : 0;
}
export function buildWorkItems(status) {
    ...
    for (const issue of status.issues) {
        ...
        const bucket = latestState === "failed"
            ? "failed"
    ...
    const newestRecentByKey = new Map();
    for (const event of status.recent_events) {
        if (!event.issue_key || event.state === "skipped")
            continue;
        ...
        const nonFailedBreaksTie = current != null && eventTs === currentTs && current.state === "failed" && event.state !== "failed";
        if (!current || eventTs > currentTs || nonFailedBreaksTie) {
            newestRecentByKey.set(event.issue_key, event);
        }
    }
    for (const event of status.recent_events) {
        if (event.state !== "failed" || !event.delivery_id || seen.has(event.delivery_id))
            continue;
...
function orphanLiveItem(key, event, inflightOnly) {
    return {
        key,
...
function latestEventFromRecent(event) {
    return {
...
function latestEventFromRunning(event) {
    return {
        delivery_id: event.delivery_id,
        event_type: event.event_type,
        state: "running",
