...
scripts/session-stats/
  ...
  burn.ts          # live burn radar: tails sessions, fires waste alerts in real time
  burn.test.ts     # detector contract tests (bun test)
```

## Live burn radar (`burn.ts`)

Where `audit.ts` is post-hoc, the burn radar is live: it tails session
transcripts as they are written and fires waste-signature alerts while the
session is still running — read-loops (same file read N times with no
intervening edit), edit-churn (N failed edits on one file), error-storms,
bloat results, burn-rate, cost-budget crossings, stalls, and compactions.

```sh
bun run stats:burn                                   # watch all active sessions
bun run stats:burn -- --once                         # one-shot report over recent sessions
bun run stats:burn -- --once --since 3d --quiet       # report, alerts only
bun run stats:burn -- --interval 2s --burn-rate 20000 --bloat 15000
```

Alerts print as one-line human summaries and append to an append-only JSONL
ledger (`~/.omp/session-burn-ledger.jsonl` by default; `--ledger` to move it,
`--no-ledger` to disable). Thresholds are all CLI-tunable; see
`bun scripts/session-stats/burn.ts --help`.

Detector contract tests: `bun test scripts/session-stats/burn.test.ts`
