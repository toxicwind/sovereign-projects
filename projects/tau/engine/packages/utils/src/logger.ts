...
import { APP_NAME, CONFIG_DIR_NAME, getLogsDir } from "./dirs";
...
const PROCESS_LOG_PATTERN = new RegExp(`^(?:omp|${APP_NAME})\\.(\\d{4}-\\d{2}-\\d{2})\\.(\\d+)\\.log(?:\\.(\\d+))?$`);
const PROCESS_AUDIT_PATTERN = new RegExp(`^(?:\\.omp|\\${CONFIG_DIR_NAME})\\.(\\d+)-audit\\.json$`);
...
function makeFileTransport(dir?: string): RotatingFileSink {
	...
	return new RotatingFileSink({
		...
		maxFiles: 5,
		auditFile: path.join(logsDir, `${CONFIG_DIR_NAME}.${process.pid}-audit.json`),
	});
}
