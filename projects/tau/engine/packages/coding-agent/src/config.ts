 ...
 * Project-level: .tau, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".tau", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.tau/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.tau/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.tau/agent/commands", source: ".tau", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.tau, .claude) - the nearest one found.
