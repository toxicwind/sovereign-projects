...
export function stream<TApi extends Api>(
	...
	// Fail fast on configuration errors with the same synchronous contract as
	// streamDispatch: the hedged wrapper below runs attempts asynchronously, so
	// without this a misconfigured call would surface as a rejected result
	// promise instead of throwing here (e.g. explicit prompt caching on an
	// unsupported Responses surface).
	assertExplicitOpenAIResponsesPromptCacheSupport(
		model,
		withTransportFetch(model, (options || {}) as StreamOptions) as OptionsForApi<TApi>,
	);
