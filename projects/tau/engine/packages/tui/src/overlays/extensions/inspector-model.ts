...
export interface ExtensionInspectorSource {
	readToolHeader?(filePath: string): string | undefined;
	/** Optional application parser for raw frontmatter; otherwise fields must already be normalized. */
	parseRule?(raw: Record<string, unknown>): {
		condition?: string[];
		astCondition?: string[];
		scope?: string[];
		agents?: string[];
...
export function toolInspectorData(
	...
	source?: ExtensionInspectorSource,
): {
	label?: string;
	description?: string;
	...
	if (lives.length > 1) {
		...
		const description = isPlaceholderToolDescription(ext.name, discovered)
			? source?.readToolHeader?.(ext.path)
			: discovered;
		return {
	...
	const description =
		live?.description && (isPlaceholderToolDescription(ext.name, discovered) || !discovered)
			? live.description
			: (discovered ?? live?.description ?? source?.readToolHeader?.(ext.path));
	return {
...
export function ruleInspectorData(
	ext: Extension,
	source?: ExtensionInspectorSource,
): {
	description?: string;
	...
	const parsed = source?.parseRule?.(raw) ?? raw;
	...
	return {
		...
		alwaysApply,
		globs,
		condition: stringArray(parsed.condition),
		astCondition: stringArray(parsed.astCondition),
		scope: stringArray(parsed.scope),
		agents: stringArray(parsed.agents),
		interruptMode,
		content,
