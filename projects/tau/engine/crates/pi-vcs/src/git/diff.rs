	...
	match prepared.operation {
		...
		gix::diff::blob::platform::prepare_diff::Operation::InternalDiff { algorithm } => {
			...
			let unified = gix::diff::blob::UnifiedDiff::new(
				&input,
				GitHunks { out: &mut hunks, old_data, new_data, added: 0, removed: 0, budget },
...
impl gix::diff::blob::unified_diff::ConsumeHunk for GitHunks<'_> {
	...
	fn consume_hunk(
		...
		for piece in hunk.split_inclusive(|byte| *byte == b'\n') {
			...
			// exact cost when the piece is already valid UTF-8) before
...
mod tests {
	...
	fn hunk_writer_stops_within_budget_instead_of_finishing_the_hunk() {
		use gix::diff::blob::unified_diff::ConsumeHunk;

		let mut out = String::new();
		// 10,000 added lines, each rendered as `+x\n` by the hunk writer.
		let hunk: Vec<u8> = (0..10_000).flat_map(|_| b"+x\n".iter().copied()).collect();
		let full_len = hunk.len();
		let budget = Some(RenderBudget { limit: 100, already: 0 });
		let mut sink = GitHunks { out: &mut out, old_data: b"", new_data: b"", added: 0, removed: 0, budget };
		let err = sink.consume_hunk(1, 0, 1, 10_000, "", &hunk).unwrap_err();
		assert!(
			err.get_ref()
				.is_some_and(|inner| inner.downcast_ref::<BudgetExceeded>().is_some()),
			"{err:?}"
	...
	fn hunk_writer_bounds_a_single_line_of_invalid_utf8_before_converting_it() {
		...
		let mut hunk = Vec::with_capacity(invalid_line.len() + 1);
		hunk.push(b'+');
		hunk.extend_from_slice(&invalid_line);
		let budget = Some(RenderBudget { limit: 100, already: 0 });
		let mut sink = GitHunks { out: &mut out, old_data: b"", new_data: b"", added: 0, removed: 0, budget };
		let err = sink.consume_hunk(1, 0, 1, 1, "", &hunk).unwrap_err();
		assert!(
			err.get_ref()
				.is_some_and(|inner| inner.downcast_ref::<BudgetExceeded>().is_some()),
			"{err:?}"
	...
	fn hunk_writer_bounds_the_function_context_line_before_converting_it() {
		...
		let budget = Some(RenderBudget { limit: 100, already: 0 });
		let mut sink = GitHunks { out: &mut out, old_data: &old_data, new_data: b"", added: 0, removed: 0, budget };
		// `before_hunk_start: 2` makes `function_context` scan exactly the
		// first line of `old_data` — the pathological one — and return it.
		let err = sink.consume_hunk(2, 0, 1, 1, "", b"+x\n").unwrap_err();
		assert!(
			err.get_ref()
				.is_some_and(|inner| inner.downcast_ref::<BudgetExceeded>().is_some()),
			"{err:?}"
