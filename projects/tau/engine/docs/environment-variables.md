...
| `GROQ_API_KEY`                  | Groq auth                                        | Using Groq models                                              | `Authorization: Bearer ${GROQ_API_KEY}` via `getEnvApiKey("groq")`; set as `${GROQ_API_KEY}` in `.env`/`settings.json`/`config.yaml` per AGENTS.md #13; baseUrl `https://api.groq.com/openai/v1` |
