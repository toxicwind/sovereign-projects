...
## Verdict
**Status: Healthy & Operational.** The fork suffered from configuration drift, dead model endpoints in subagent defaults, restrictive advisor tool whitelisting, and a split between `~/.pi` and canonical `~/.tau` paths. With model routing re-anchored to active 1M-context models (`gemini-3.7-flash-tiered`, local `llama-swap`), thinking efforts corrected, and launcher CWD-awareness restored, the agent runtime is fully functional across native and MCP tools.
...
- **Cause**: `~/.tau/agent/config.yml` had `smol: fastrouter/nvidia/nemotron-3-super:free:auto`. That free fastrouter endpoint was decommissioned.
...
### 5. `~/.pi` vs `~/.tau` State Split
- **Symptom**: Conflicting settings and blocked tools.
- **Cause**: `pi-utils` resolves configuration to `~/.tau/agent/`. An ad-hoc `~/.pi/agent/config.yaml` locked `allowed-tools` to gateway-only calls.
- **Fix**: Synchronized `~/.pi` with `~/.tau` and restored native tool declarations.
