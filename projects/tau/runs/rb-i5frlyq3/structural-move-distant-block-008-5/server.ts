...
export interface IpcRequestHandler {
  ...
  ): Promise<StatusResponse | ErrorResponse> | StatusResponse | ErrorResponse;
  (
    request: RpcRequest,
  ): Promise<RpcBridgeResponse | ErrorResponse> | RpcBridgeResponse | ErrorResponse;
  (
    request: RpcStreamRequest,
  ): Promise<RpcReadyResponse | ErrorResponse> | RpcReadyResponse | ErrorResponse;
  ...
  openRpcStream(
    ...
    | {
        handleRequest(
          request: RpcRequest['command'] | { type: 'extension_ui_response' },
        ): Promise<void>;
        close(): void;
      }
    | undefined;
          ...
          if (!rpcStream) {
            socket.end(
              encodeMessage({
                type: 'error',
                ok: false,
                error: `Unknown instance: ${request.instanceId}`,
              }),
            );
            return;
          }
