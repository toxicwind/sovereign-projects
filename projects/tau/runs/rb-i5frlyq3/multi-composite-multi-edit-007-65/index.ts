...
async function hasExistingChild(
  db: SqliteDatabase,
  sessionId: string,
  ...
  const row =
    parentId === null
      ? await db
          .prepare(
            'SELECT 1 AS found FROM session_entries WHERE session_id = ? AND parent_id IS NULL LIMIT 1',
          )
          .get<{ found: number }>(sessionId)
      : await db
          .prepare(
            'SELECT 1 AS found FROM session_entries WHERE session_id = ? AND parent_id = ? LIMIT 1',
          )
...
async function loadSqliteStorage(
  ...
  const row = await db
    .prepare(
      'SELECT id, created_at, metadata, cwd, parent_session_id, active_leaf_id FROM sessions WHERE id = ?',
    )
...
export class SqliteSessionStorage implements SessionStorage<SqliteSessionMetadata> {
  ...
  private async getPathToRootOrCompactionEntries(
    leafId: string | null,
  ...
  private async materializeBranch(leafId: string | null): Promise<void> {
    ...
    for (const entry of path) {
      ...
      if (!entryRow)
        throw invalidSession(`missing entry row for session ${this.metadata.id} entry ${entry.id}`);
      await this.db
        .prepare(
  ...
  private async appendToActiveBranch(entryId: string, parentId: string | null): Promise<void> {
    ...
    if (entryRow)
      throw invalidSession(`missing entry row for session ${this.metadata.id} entry ${entryId}`);
    await this.db
      .prepare(
        'INSERT INTO branch_entries (session_id, branch_id, entry_id, entry_seq) VALUES (?, ?, ?, ?)',
      )
  ...
  async appendEntry(entry: SessionTreeEntry): Promise<void> {
    ...
    const previousMaterializedState: SessionMaterializedState = {
      ...
      currentModel: this.materializedState.currentModel
        ? { ...this.materializedState.currentModel }
        : null,
    };
    ...
    try {
      ...
      await this.db.transaction(async () => {
        const parentHadExistingChild = await hasExistingChild(
          this.db,
          this.metadata.id,
          entry.parentId,
        );
        ...
        for (const materializedEntry of entryMaterializedValues(entry)) {
          await this.db
            .prepare(
              'INSERT INTO entry_materialized (session_id, entry_seq, type, payload) VALUES (?, ?, ?, ?)',
            )
    ...
    if (leafId === this.currentLeafId) {
      if (!this.activeBranchId) {
        throw invalidSession(
          `missing active branch for session ${this.metadata.id} leaf ${leafId}`,
        );
      }
      return getMaterializedBranchPathOrCompaction(
        this.db,
        this.metadata.id,
        this.activeBranchId,
        this.byId,
    ...
    if (limit !== undefined) {
      const beforeOrAtEntrySeq =
        ...
        (
          await this.db
            .prepare(
              'SELECT entry_seq FROM session_entries WHERE session_id = ? ORDER BY entry_seq DESC LIMIT 1',
            )
