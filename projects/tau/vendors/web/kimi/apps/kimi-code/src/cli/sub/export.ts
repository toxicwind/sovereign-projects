...
// Tau bridge imports (maximal connect to oh-my-pi engine)
import { createTauHarness, TauHarness } from '@moonshot-ai/tau-bridge';
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps {
  let harness: TauHarness | undefined;
  ...
  const getHarness = async (): Promise<TauHarness> => {
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    if (!harness) {
      harness = await createTauHarness({
  ...
  const initializeDefaultTelemetry = async (): Promise<void> => {
    ...
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    const currentHarness = await getHarness();
  ...
  return {
    listSessions:
      ...
      ((workDir: string) =>
        getHarness().then(h => h.listSessions({ workDir }))),
    exportSession:
      overrides.exportSession ??
      (async (input: ExportSessionInput) => {
        await initializeDefaultTelemetry();
        try {
          return await getHarness().then(h => h.exportSession(input));
        } finally {
