...
function ensureKimiPropertyTypes(schema: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!isRecord(normalized)) {
    throw new SyntaxRequestFormatError('JSON Schema root must normalize to an object.');
  }
...
function inferTypeFromValues(values: unknown[]): JsonSchemaType {
  ...
  if (types.length === 1) {
    ...
    if (onlyType === undefined) {
      throw new SyntaxRequestFormatError('Cannot infer JSON Schema type from an empty enum.');
    }
    return onlyType;
  }
  throw new SyntaxRequestFormatError(
