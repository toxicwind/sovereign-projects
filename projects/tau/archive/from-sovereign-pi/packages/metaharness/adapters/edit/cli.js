...
import { buildBenchmarkResult, runBenchmark } from "./runner";
...
function parseThinkingLevel(value) {
    if ([ThinkingLevel.Off, ...THINKING_EFFORTS].includes(value)) {
        return value;
    }
    return fail(`Invalid thinking level: ${value}. Valid levels: ${[ThinkingLevel.Off, ...THINKING_EFFORTS].join(", ")}`);
}
function parsePositiveInt(raw, flag, fallback) {
    if (raw === undefined)
        return fallback;
    const parsed = Number.parseInt(raw, 10);
    if (Number.isNaN(parsed) || parsed < 1)
        fail(`Invalid ${flag} value: ${raw}. Must be a positive integer.`);
    return parsed;
}
function parseFuzzy(raw) {
    if (raw === undefined)
        return undefined;
    if (raw === "auto")
        return "auto";
...
function parseFuzzyThreshold(raw) {
    if (raw === undefined)
        return undefined;
    if (raw === "auto")
        return "auto";
...
function resolveFormat(explicit, outputPath) {
    if (explicit === "markdown" || explicit === "json")
        return explicit;
    if (explicit !== undefined)
        fail(`Invalid --format: ${explicit}. Must be markdown or json.`);
    return outputPath.endsWith(".md") || outputPath.endsWith(".markdown") ? "markdown" : "json";
}
function generateReportFilename(config) {
...
async function resolveConversationDumpDir(outputPath) {
...
async function resolveFixtures(fixturesArg) {
    ...
    try {
        if (values["check-fixtures"]) {
            ...
            for (const issue of issues)
                log(`  - ${issue.taskId}: ${issue.message}`);
            process.exitCode = 1;
            return;
        }
        ...
        if (!values.model)
            fail("edit adapter requires --model (see --help)");
        if (values.tasks) {
            ...
            if (tasks.length !== selected.size)
                fail("one or more edit task ids were not found (see --list)");
        }
        else {
        ...
        const config = {
            ...
            timeout: parsePositiveInt(values.timeout, "--timeout", 120000),
            connectionTimeout: parsePositiveInt(values["connection-timeout"], "--connection-timeout", 30000),
        ...
        if (editVariant)
            config.editVariant = editVariant;
        const editFuzzy = parseFuzzy(values["edit-fuzzy"]);
        if (editFuzzy !== undefined)
            config.editFuzzy = editFuzzy;
        const editFuzzyThreshold = parseFuzzyThreshold(values["edit-fuzzy-threshold"]);
        if (editFuzzyThreshold !== undefined)
            config.editFuzzyThreshold = editFuzzyThreshold;
        ...
        if (config.editVariant)
            log(`Edit variant: ${config.editVariant}`);
        if (config.editFuzzy !== undefined)
            log(`Edit fuzzy: ${config.editFuzzy}`);
        if (config.editFuzzyThreshold !== undefined)
            log(`Edit fuzzy threshold: ${config.editFuzzyThreshold}`);
        log(`Tasks: ${tasks.length}`);
        log(`Report: ${outputPath}`);
        log(`Conversation dumps: ${config.conversationDumpDir}`);
        log();
        const renderReport = (result) => format === "json" ? generateJsonReport(result) : generateReport(result);
        ...
        const result = await runBenchmark(tasks, config, event => progress?.handleEvent(event), snapshot => {
        ...
        log(`  Task success rate (best of ${config.runsPerTask}): ${(result.summary.taskSuccessRate * 100).toFixed(1)}% (${result.summary.successfulTasks}/${result.summary.totalTasks})`);
        log(`  Total tokens (best): ${result.summary.totalTokens.input} in / ${result.summary.totalTokens.output} out`);
        log(`  Tokens/task (best): mean=${result.summary.avgTokensPerTask.total} median=${result.summary.medianTokensPerTask.total} p1=${result.summary.p1TokensPerTask.total} p99=${result.summary.p99TokensPerTask.total} reasoning=${result.summary.avgTokensPerTask.reasoning}`);
        if (result.summary.ghostRuns > 0)
            log(`  Ghost runs (0/0/0): ${result.summary.ghostRuns}`);
        if (result.summary.timeoutRuns > 0)
            log(`  Timeout runs: ${result.summary.timeoutRuns}`);
