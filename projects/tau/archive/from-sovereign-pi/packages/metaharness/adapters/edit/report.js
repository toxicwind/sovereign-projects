...
import { EDIT_FAILURE_CATEGORIES } from "./runner";
function formatBestStatus(task, runsPerTask) {
...
function formatEditArgsBlock(args) {
...
export function generateReport(result) {
    ...
    const nonGhostRuns = allRuns.filter(run => run.success ||
        run.tokens.total > 0 ||
    ...
    lines.push("## Summary");
    lines.push("");
    lines.push("Primary metrics (tokens, duration, tool calls) are aggregated over the **best run** of each task. Diagnostic counts (ghost runs, timeouts, retries, failure categories) span every executed run.");
    lines.push("");
    ...
    if (config.editVariant === "hashline") {
        lines.push(`| **Autocorrect-Free Success Rate** | **${formatRate(summary.autocorrectFreeSuccessfulTasks, summary.totalTasks)}** |`);
    ...
    lines.push(`| **Tool Input Chars** | ${formatNumber(summary.totalToolCalls.totalInputChars)} | ${formatNumber(Math.round(summary.avgToolCallsPerTask.totalInputChars))} |`);
    lines.push("");
    lines.push("### Tokens & Time (Overall)");
    lines.push("");
    lines.push("| Metric | Total (best) | Avg/Task | Median | P1 | P99 |");
    lines.push("|--------|--------------|----------|--------|----|----|");
    lines.push(`| Input Tokens | ${formatNumber(summary.totalTokens.input)} | ${formatNumber(summary.avgTokensPerTask.input)} | ${formatNumber(summary.medianTokensPerTask.input)} | ${formatNumber(summary.p1TokensPerTask.input)} | ${formatNumber(summary.p99TokensPerTask.input)} |`);
    lines.push(`| Output Tokens | ${formatNumber(summary.totalTokens.output)} | ${formatNumber(summary.avgTokensPerTask.output)} | ${formatNumber(summary.medianTokensPerTask.output)} | ${formatNumber(summary.p1TokensPerTask.output)} | ${formatNumber(summary.p99TokensPerTask.output)} |`);
    lines.push(`| Reasoning Tokens | ${formatNumber(summary.totalTokens.reasoning)} | ${formatNumber(summary.avgTokensPerTask.reasoning)} | ${formatNumber(summary.medianTokensPerTask.reasoning)} | ${formatNumber(summary.p1TokensPerTask.reasoning)} | ${formatNumber(summary.p99TokensPerTask.reasoning)} |`);
    lines.push(`| Total Tokens | ${formatNumber(summary.totalTokens.total)} | ${formatNumber(summary.avgTokensPerTask.total)} | ${formatNumber(summary.medianTokensPerTask.total)} | ${formatNumber(summary.p1TokensPerTask.total)} | ${formatNumber(summary.p99TokensPerTask.total)} |`);
    lines.push(`| Duration | ${formatDuration(summary.totalDuration)} | ${formatDuration(summary.avgDurationPerTask)} | — | — | — |`);
    ...
    lines.push("|--------|-------|----------|--------|----|----|");
    lines.push(`| Input Tokens | ${formatNumber(summary.totalOneShotSuccessTokens.input)} | ${formatNumber(summary.avgOneShotSuccessTokensPerTask.input)} | ${formatNumber(summary.medianOneShotSuccessTokensPerTask.input)} | ${formatNumber(summary.p1OneShotSuccessTokensPerTask.input)} | ${formatNumber(summary.p99OneShotSuccessTokensPerTask.input)} |`);
    lines.push(`| Output Tokens | ${formatNumber(summary.totalOneShotSuccessTokens.output)} | ${formatNumber(summary.avgOneShotSuccessTokensPerTask.output)} | ${formatNumber(summary.medianOneShotSuccessTokensPerTask.output)} | ${formatNumber(summary.p1OneShotSuccessTokensPerTask.output)} | ${formatNumber(summary.p99OneShotSuccessTokensPerTask.output)} |`);
    ...
    lines.push(`| Total Tokens | ${formatNumber(summary.totalOneShotSuccessTokens.total)} | ${formatNumber(summary.avgOneShotSuccessTokensPerTask.total)} | ${formatNumber(summary.medianOneShotSuccessTokensPerTask.total)} | ${formatNumber(summary.p1OneShotSuccessTokensPerTask.total)} | ${formatNumber(summary.p99OneShotSuccessTokensPerTask.total)} |`);
    lines.push("");
    ...
    const totalCategorizedEditFailures = EDIT_FAILURE_CATEGORIES.reduce((sum, category) => sum + (summary.editFailureCategories[category] ?? 0), 0);
    ...
    for (const task of tasks) {
        ...
        lines.push(`| ${escapeMarkdown(task.name)} | ${escapeMarkdown(formatFiles(task.files))} | ${label} ${status} | ${editHitRate} | ${toolCalls} | ${formatNumber(task.tokens.input)}/${formatNumber(task.tokens.output)} | ${formatDuration(task.duration)} | ${formatScore(task.indentScore)} |`);
    }
    lines.push("");
    ...
    if (flakyTasks.length > 0) {
        ...
        for (const task of flakyTasks) {
            ...
            for (const run of task.runs) {
                ...
                lines.push(`| ${run.runIndex + 1}${marker} | ${status} | ${error} | ${formatNumber(run.tokens.input)} / ${formatNumber(run.tokens.output)} | ${formatDuration(run.duration)} |`);
            }
            lines.push("");
        }
    }
    ...
    if (failedTasks.length > 0) {
        ...
        for (const task of failedTasks) {
            ...
            if (uniqueErrors.length === 1) {
                ...
                for (const run of task.runs) {
                    const error = run.error ? truncate(escapeMarkdown(run.error), 50) : "—";
                    lines.push(`| ${run.runIndex + 1} | ❌ | ${error} | ${formatNumber(run.tokens.input)} / ${formatNumber(run.tokens.output)} | ${formatDuration(run.duration)} |`);
                }
            }
            lines.push("");
...
function appendCategorySummary(lines, tasks) {
    ...
    for (const category of categories) {
        ...
        const scores = difficultyByCategory.get(category) ?? [];
        lines.push(`| ${escapeMarkdown(category)} | ${entry.runs} | ${formatRate(entry.verified, entry.runs)} | ${formatRate(entry.editUsed, entry.runs)} | ${formatRate(entry.success, entry.runs)} | ${formatDifficultyStats(scores)} |`);
    }
    lines.push("");
}
function appendMutationSummary(lines, tasks) {
    const runs = tasks.flatMap(task => task.runs);
    ...
    for (const mutation of mutations) {
        const entry = mutationStats.get(mutation);
        lines.push(`| ${escapeMarkdown(mutation)} | ${escapeMarkdown(entry.category)} | ${entry.runs} | ${formatRate(entry.verified, entry.runs)} | ${formatRate(entry.editUsed, entry.runs)} | ${formatRate(entry.success, entry.runs)} |`);
    }
    lines.push("");
}
function appendDifficultySummary(lines, tasks) {
    const runs = tasks.flatMap(task => task.runs);
    const buckets = [
    ...
    for (const bucket of buckets) {
        ...
        lines.push(`| ${bucket.label} | ${entry.runs} | ${formatRate(entry.verified, entry.runs)} | ${formatRate(entry.editUsed, entry.runs)} | ${formatRate(entry.success, entry.runs)} |`);
    }
    if (unknown.runs > 0) {
        lines.push(`| unknown | ${unknown.runs} | ${formatRate(unknown.verified, unknown.runs)} | ${formatRate(unknown.editUsed, unknown.runs)} | ${formatRate(unknown.success, unknown.runs)} |`);
    }
    lines.push("");
}
function formatDifficultyStats(scores) {
    if (scores.length === 0)
        return "—";
