...
const VOCAB_SIZE = 248044; // base vocab; 33 added tokens (248044-248076) excluded
...
if (model.byte_fallback || model.ignore_merges)
    throw new Error("unexpected model flags");
if (tj.normalizer?.type !== "NFC")
    throw new Error("expected NFC normalizer");
...
const u2b = {};
{
    ...
    for (let i = 0; i < bs.length; i++)
        u2b[String.fromCodePoint(cs[i])] = bs[i];
}
...
const entries = new Array(Object.keys(vocab).length).fill(null);
let dead = 0;
for (const tok in vocab) {
    const rank = vocab[tok];
    if (entries[rank] !== null)
    ...
    for (let i = 0; i < chars.length; i++) {
        const b = u2b[chars[i]];
        if (b === undefined)
            throw new Error(`non-alphabet char in vocab entry ${rank}: ${tok}`);
        bytes[i] = b;
    }
    entries[rank] = bytes;
}
// Assertions: size + rank contiguity (no null slot).
if (entries.length !== VOCAB_SIZE)
    throw new Error(`vocab size ${entries.length}, expected ${VOCAB_SIZE}`);
for (let r = 0; r < entries.length; r++) {
    if (entries[r] === null)
        throw new Error(`rank gap at ${r}: ranks not contiguous`);
}
