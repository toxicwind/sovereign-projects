...
> **SUPERSEDED 2026-09-19 — READ FIRST.** `config.yaml` was regenerated by
> autoscan after this audit, so the config-state claims below no longer match
> the current file (verified 2026-09-19):
> - `routing.scheduler.settings.fifo.priority` is now **empty** (`{}`) — the
>   "36 entries, dense unique ladder 0–35" claim is stale.
> - `routing.router.settings.matrix.vars` / `evict_costs` are now **empty**;
>   `sets.exclusive` is an empty string.
> - Model IDs were renamed to a file-derived scheme (e.g. `gemma-4-12b-it-q4_k_m`);
>   none of the `beellama/*`, `qwen/27b-*`, `turboquant/*`, `ik_llama/*` IDs
>   referenced below exist in the current config. The "Fixed in this pass"
>   table documents fixes to that pre-autoscan config.
> - Macros `SPEC_DFLASH_QWEN`, `SPEC_DFLASH_GEMMA`, `MMPROJ_27B_F16`,
>   `QWEN36_27B_UD_Q4XL` no longer exist in the current `macros:` block.
> - Model files live in `/home/toxic/models` (21 `.gguf` files), **not**
>   `/home/toxic/projects/models` (that directory does not exist).
> The audit body is kept below as history.
