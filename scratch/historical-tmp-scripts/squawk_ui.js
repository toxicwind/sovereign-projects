...
let pollCtrl = null; // per-generation AbortController: a channel switch aborts in-flight polls
...
async function poll(myGen) {
  ...
  try {
    const r = await fetch(`wait?since=${since}&channel=${chan()}`, { headers: auth(), signal: AbortSignal.any([pollCtrl.signal, AbortSignal.timeout(65000)]) });
    if (r.status === 401 || r.status === 404) throw new Error('unauthorized');
    const data = await r.json();
    if (myGen !== gen) return; // channel switched mid-flight: drop the stale response
    failures = 0;
    ...
    if (myGen !== gen) return; // superseded: the new generation owns the loop now
    failures++;
...
async function boot() {
  const myGen = ++gen;
  if (pollCtrl) { try { pollCtrl.abort(); } catch (e) {} } // kill the old generation in flight
  ...
  try {
    const r = await fetch(`wait?since=0&tail=${TAIL}&channel=${chan()}`, { headers: auth(), signal: AbortSignal.any([pollCtrl.signal, AbortSignal.timeout(65000)]) });
    if (r.status === 401 || r.status === 404) throw new Error('unauthorized');
    const data = await r.json();
