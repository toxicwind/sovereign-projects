...
lines = [json.dumps({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"b","version":"1"}}}),
         ...
         json.dumps({"jsonrpc":"2.0","id":10,"method":"tools/call","params":{"name":"initialize_browserless","arguments":{"token":""}}}),
...
p = subprocess.Popen(["node","dist/index.js"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True, cwd="/tmp/browserless-fix/projects/mesh/browserless")
p.stdin.write("\n".join(lines)+"\n"); p.stdin.flush()
...
for i in [10,11,12,13,14]:
    d=got.get(i,{})
    if "result" in d:
        txt=json.dumps(d["result"])[:120]
        print(f"ID {i} PASS: {txt}")
    else:
        print(f"ID {i} FAIL: {json.dumps(d.get(error,{}))[:120]}")
p.kill()
