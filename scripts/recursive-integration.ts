...
async function run() {
  ...
  const allProjects = (await $`ls -1 ${PROJECTS}`.text()).split('\n').filter(Boolean);
  ...
  const marker = {
    ...
    sovereign: { path: SOVEREIGN, helpers: ["git-mutator", "ast-migrate", "health-audit", "mesh-probe", "safe-rg-audit", "hardware-telemetry", "clean-orphans"] },
    sovereignProjects: { path: SOV_PROJECTS, subdirs: spContents.trim().split('\n').filter(Boolean) },
    projects: { path: PROJECTS, total: allProjects.length, relevant: relevant.length, samples: relevant.slice(0, 10) }
  };
  
  await Bun.write(`${ROOT}/.recursive-integration.json`, JSON.stringify(marker, null, 2));
