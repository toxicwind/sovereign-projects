...
# ============================================================
# SOVEREIGN DEPLOYMENT – AWK-based CMake patch with correct path
# ============================================================
...
IK_LLAMA_SRC="${IK_LLAMA_SRC:-/home/toxic/ik_llama.cpp-main}"
BUILD_DIR="${IK_LLAMA_SRC}/build"
INSTALL_BIN="${SOVEREIGN_HOME}/bin/llama-server"
FLOW_FILE="${SOVEREIGN_HOME}/nfcot_flow.pt"

echo "============================================================"
echo " SOVEREIGN DEPLOYMENT (AWK patching, verified path)"
echo "============================================================"

# 0. Ensure directories exist
mkdir -p "${SOVEREIGN_HOME}/modules" "${SOVEREIGN_HOME}/bin" "${SOVEREIGN_HOME}/logs"
...
# 2. Install proxy (unchanged, included for completeness)
...
# 3. Patch CMakeLists.txt with awk (correct path: src/sovereign/)
echo "3. Patching CMakeLists.txt with awk (path: src/sovereign/...)..."
cd "${IK_LLAMA_SRC}"

# Remove any prior erroneous patches (lines containing 'sovereign_governor.cpp' without path)
if grep -q "target_sources(llama PRIVATE sovereign_governor.cpp" CMakeLists.txt; then
    echo "   Removing previous incorrect patch..."
    sed -i '/target_sources(llama PRIVATE sovereign_governor.cpp/d' CMakeLists.txt
    sed -i '/target_include_directories(llama PRIVATE ${CMAKE_CURRENT_SOURCE_DIR})/d' CMakeLists.txt
    sed -i '/# Sovereign governor integration/d' CMakeLists.txt
fi

# Check if already correctly patched
if grep -q "src/sovereign/sovereign_governor.cpp" CMakeLists.txt; then
    echo "   CMakeLists.txt already correctly patched."
else
    # Use awk to insert after add_library(llama ...)
    awk '
        /add_library\(llama/ { print; print "# Sovereign governor integration (src/sovereign)";
                               print "target_sources(llama PRIVATE src/sovereign/sovereign_governor.cpp src/sovereign/weight_pack_sm86_fixed.cu)";
                               print "target_include_directories(llama PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/src/sovereign)";
                               next }
        { print }
    ' CMakeLists.txt > CMakeLists.txt.new
    mv CMakeLists.txt.new CMakeLists.txt
    echo "   CMakeLists.txt patched with awk."
fi

# 4. Rebuild ik_llama
echo "4. Rebuilding ik_llama with Sovereign governor..."
mkdir -p "${BUILD_DIR}"
cd "${BUILD_DIR}"
...
# Copy binary
cp bin/llama-server "${INSTALL_BIN}" 2>/dev/null || cp bin/llama-cli "${INSTALL_BIN}" 2>/dev/null || {
    echo "   Error: Could not find built binary. Check build output."
    exit 1
}
echo "   Binary installed to ${INSTALL_BIN}"

# 5. Ensure flow file exists
if [ ! -f "${FLOW_FILE}" ]; then
    echo "5. Creating default flow file (dim=5120)..."
...
# 6. Start only the core services
echo "6. Starting stack with mise (core only)..."
cd "${SOVEREIGN_HOME}"
mise run llama-server ::: nfcot-proxy &

# 7. Wait and verify
echo "7. Waiting for services to come up..."
sleep 5
