...
const STRATEGIES: SearchStrategy[] = [
  ...
  {
    name: "ast-grep-convert",
    q: "ast-grep+convert+OR+migrate+OR+transform",
    w: 0.85,
  },
  { name: "ast-grep-codemod", q: "ast-grep+codemod+OR+refactor", w: 0.8 },
  {
    name: "ast-grep-patterns",
    q: "ast-grep+pattern+OR+snippet+OR+example",
    w: 0.7,
  },
  {
    name: "ast-grep-presets",
    q: "ast-grep+preset+OR+collection+OR+pack",
    w: 0.75,
  },
  {
    name: "ast-grep-config",
    q: "ast-grep+config+OR+sgconfig+OR+setup",
    w: 0.6,
  },
  {
    name: "ast-grep-bun",
    q: "ast-grep+bun+OR+nodejs+OR+typescript+rules",
    w: 0.65,
  },
...
async function checkRulesRepo(
  fullName: string,
): Promise<{ hasRules: boolean; ruleCount: number }> {
  ...
  const ruleFiles = (contents as any[]).filter(
    (f: any) =>
      f.name.endsWith(".yml") ||
      f.name.endsWith(".yaml") ||
      f.name === "rules" ||
      (f.type === "dir" && f.name.includes("rule")),
  );
...
function scoreRepo(
  ...
  ruleInfo: RepoScore["hasRules"] | any,
): RepoScore {
...
async function main() {
  ...
  console.log(
    `Target: ${targetCount} repos | Strategies: ${STRATEGIES.length}`,
  );
  ...
  for (const strategy of STRATEGIES) {
    ...
    for (const item of items) {
      ...
      console.log(
        `  + ${item.full_name} (score: ${scored.score}, stars: ${scored.stars}, rules: ${ruleInfo.hasRules})`,
      );
    }
  }
  ...
  top.forEach((r, i) => {
    ...
    console.log(
      `   Score: ${r.score} | Stars: ${r.stars} | Rules: ${r.hasRules} (${r.ruleCount})`,
    );
  ...
  console.log(
    `Saved to audit/discovery_${topic.replace(/[^a-z0-9]/gi, "_")}.json`,
  );
}
