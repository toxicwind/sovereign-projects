...
let
  ...
  llama-herder-pkg = pkgs.python3Packages.buildPythonApplication {
    pname = "llama-herder";
    version = "0.1.0";
    src = if builtins.pathExists "${SOV_HOME}/tools/llamaherder"
  ...
  configToml = pkgs.writeText "config.toml" ''
    api_listen = "127.0.0.1:${toString PORTS.nfcot}"
    auto_approve = true
...
in
{
  ...
  packages = with pkgs; [
    ...
    python3Packages.mypy python3Packages.pyright
  ...
  env = {
    ...
    NIXPKGS_CONFIG = "{ allowUnfree = true; cudaSupport = true; }";
  ...
  # ═══════════════════════════════════════════════════════════════════
  # SERVICES — All devenv native services enabled
  # ═══════════════════════════════════════════════════════════════════
  services = {
    # Web server
    caddy = {
    ...
    mysql = {
      ...
      initialDatabases = [{ name = "sovereign"; }];
      initialScript = ''
        CREATE USER 'sovereign'@'localhost' IDENTIFIED BY 'sovereign';
        GRANT ALL PRIVILEGES ON sovereign.* TO 'sovereign'@'localhost';
      '';
    };
    ...
    clickhouse = {
      ...
      http_port = PORTS.clickhouse-http;
    };
    ...
    minio = {
      enable = true;
      port = PORTS.minio;
      consolePort = PORTS.minio-console;
    ...
    trafficserver = {
      enable = true;
      port = PORTS.trafficserver;
    };

    # Profiling
    blackfire = {
      enable = true;
      socket = "tcp://127.0.0.1:${toString PORTS.blackfire}";
    };
  };
...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path.startswith("/hf/"):
            ...
            try:
                subprocess.run(
                    ["hf", "download", repo, "--local-dir", local_dir, "--local-dir-use-symlinks", "False"],
...
EOF
    ...
    port-map.exec = ''
      ...
      echo "║  minio         :${toString PORTS.minio}     (MinIO S3)                      ║"
      echo "║  minio-console :${toString PORTS.minio-console}     (MinIO Console)            ║"
  ...
  enterShell = ''
    ...
    echo "║  Storage:      MinIO :${toString PORTS.minio}  Console :${toString PORTS.minio-console}            ║"
    echo "╠══════════════════════════════════════════════════════════════╣"
  ...
  # ═══════════════════════════════════════════════════════════════════
  # CONTAINERS — Exportable environments
  # ═══════════════════════════════════════════════════════════════════
  containers = {
    shell = {
