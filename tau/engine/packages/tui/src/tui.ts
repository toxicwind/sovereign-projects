...
export class TUI extends Container {
	...
	#compositeOverlaysIntoWindow(window: string[], termWidth: number, termHeight: number): string[] {
		...
		for (const entry of this.overlayStack) {
			...
			for (let i = 0; i < overlayLines.length; i++) {
				...
				const rawOverlayLine = overlayLines[i];
				if (rawOverlayLine === undefined) continue;
				const truncatedOverlayLine =
					visibleWidth(rawOverlayLine) > width ? sliceByColumn(rawOverlayLine, 0, width, true) : rawOverlayLine;
	...
	#emitAltFrame(lines: string[], width: number, height: number): void {
		...
		for (let r = 0; r < height; r++) {
			if (r > 0) buffer += "\r\n";
			const fittedLine = fitted[r] ?? "";
			buffer += this.#lineRewriteSequence(fittedLine, width, r, -1, -1, this.#osc66SpacerGlyphWidth(fitted, r));
		}
		buffer += this.#paintEndSequence;
		this.terminal.write(buffer);
