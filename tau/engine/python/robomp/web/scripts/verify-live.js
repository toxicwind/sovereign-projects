...
async function main() {
    ...
    const retryResponsePromise = page.waitForResponse((resp) => new URL(resp.url()).pathname === "/api/trigger" && resp.request().method() === "POST", { timeout: 5000 });
    ...
    const retryContractOk = retryStatus === 202 &&
    ...
    const cancelResponsePromise = page.waitForResponse((resp) => new URL(resp.url()).pathname === "/api/cancel" && resp.request().method() === "POST", { timeout: 5000 });
    await page.evaluate(() => {
        window.confirm = () => true;
    ...
    const cancelOk = cancelStatus === 202 &&
