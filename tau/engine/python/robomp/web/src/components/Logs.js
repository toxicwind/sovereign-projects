...
export function Logs() {
    ...
    let scrollEl;
    const allEntries = () => logsResource()?.entries ?? [];
    const rows = () => {
        ...
        for (const entry of allEntries()) {
            ...
            if (needle) {
                const haystack = (msg +
                    " " +
                    extras.map((e) => `${e.key}=${e.value}`).join(" ")).toLowerCase();
    ...
    return (<GlassCard heading="agent logs" accessory={<span class="tabular">
      ...
      <div class="px-4 pb-3 pt-1 flex flex-wrap items-center gap-3 border-b border-stroke-soft">
        ...
        <label class="flex items-center gap-2 text-[11px] uppercase tracking-[0.14em] text-ink-400 flex-1 min-w-[200px]">
          filter
          <input type="search" value={filter()} placeholder="substring" onInput={(ev) => setFilter(ev.currentTarget.value)} class="flex-1"/>
        </label>
        <label class="flex items-center gap-2 text-[12px] text-ink-300 select-none cursor-pointer">
          <input type="checkbox" checked={follow()} onChange={(ev) => setFollow(ev.currentTarget.checked)}/>
          follow tail
        </label>
      </div>
