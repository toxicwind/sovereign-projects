...
import { activeView, setActiveView } from "../../view";
...
export function Rail() {
    return (<aside class="rmp-rail">
      ...
      <nav class="rmp-nav">
        {NAV.map((item) => (<button type="button" class="rmp-nav-item" data-active={activeView() === item.id ? "true" : "false"} data-locked={item.locked ? "true" : "false"} aria-current={activeView() === item.id ? "page" : undefined} title={item.locked ? "Triage — locked (set ROBOMP_REPLAY_TOKEN to enable)" : item.label} onClick={() => setActiveView(item.id)}>
