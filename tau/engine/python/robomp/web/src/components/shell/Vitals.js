...
export function Vitals() {
    ...
    const counts = () => {
        ...
        if (!status)
            return { queued: 0, running: 0, done: 0, failed: 0, skipped: 0 };
    ...
    return (<div class="rmp-vitals">
      {/* health / sync */}
      <div class="rmp-vital-sync" title={lastTickError() ?? undefined}>
        <span class={`rmp-vital-dot ${syncTone()}${isFetching() ? " pulse" : ""}`}/>
        <Show when={lastTickError()} fallback={<span>{syncText()}</span>}>
      ...
      <div class="rmp-vital-big">
        <div class="rmp-vital-big-cell">
          ...
          <span class={`rmp-vital-big-num${runningTotal() > 0 ? " running-nonzero" : ""}`}>
            {runningTotal()}
          </span>
        </div>
        <div class="rmp-vital-big-cell">
          <span class="rmp-vital-big-label">failed</span>
          <span class={`rmp-vital-big-num${failedCount() > 0 ? " failed-nonzero" : ""}`}>
            {failedCount()}
          </span>
        </div>
      </div>
      ...
      <div class="rmp-vital-counts" title="newest non-skipped event per issue">
        {EVENT_STATE_ORDER.map((state) => (<div class="rmp-vital-count">
            <span class="rmp-vital-count-label">{state}</span>
            <span class="rmp-vital-count-num" style={{ color: STATE_TONE[state] ?? "var(--color-ink-200)" }}>
              {counts()[state] ?? 0}
            </span>
          </div>))}
      </div>
      ...
      <div class="rmp-vital-runtime">
        ...
        <RuntimeRow label="model" value={runtime()?.model} mono title={runtime()?.thinking_level ? `thinking ${runtime()?.thinking_level}` : undefined}/>
        <RuntimeRow label="conc" value={runtime()?.max_concurrency != null
            ? String(runtime()?.max_concurrency)
            : undefined}/>
        <RuntimeRow label="up" value={runtime()?.uptime_seconds != null
...
function RuntimeRow(props) {
    return (<div class="rmp-vital-runtime-row" title={props.title}>
      <span class="rmp-vital-runtime-label">{props.label}</span>
      <span class={`rmp-vital-runtime-value${props.mono ? " mono" : ""}`} title={props.value}>
        {props.value ?? "—"}
      </span>
    </div>);
}
