...
export function Trigger(props) {
    ...
    const validate = () => {
        ...
        if (!value)
            return "enter owner/repo#NN or github issue url";
        return null;
    };
    const handleTriage = () => {
        const value = issue().trim();
    ...
    return (<Show when={variant() === "bar"} fallback={<PanelBody issue={issue} setIssue={setIssue} handleTriage={handleTriage} handleRetry={handleRetry} validate={validate}/>}>
      <Show when={CONFIG.replayEnabled} fallback={<div class="rmp-trigger-bar">
        ...
        <div class="rmp-trigger-bar">
          <input type="text" spellcheck={false} placeholder="owner/repo#NN or issue url" autocomplete="off" value={issue()} onInput={(ev) => setIssue(ev.currentTarget.value)} onKeyDown={(ev) => {
            if (ev.key === "Enter")
                handleTriage();
...
function PanelBody(p) {
    return (<GlassCard heading="trigger">
      <Show when={CONFIG.replayEnabled} fallback={<div class="px-5 py-7 text-ink-300 text-[13px] leading-relaxed">
        ...
        <div class="px-5 pb-5 pt-1 flex flex-col gap-4">
          <div class="form-row">
            <input type="text" spellcheck={false} placeholder="octo/widget#42 or https://github.com/owner/repo/issues/42" autocomplete="off" value={p.issue()} onInput={(ev) => p.setIssue(ev.currentTarget.value)} onKeyDown={(ev) => {
            if (ev.key === "Enter")
                p.handleTriage();
          ...
          <Show when={triggerStatus().text} fallback={<span class={`text-[12px] ${STATUS_TONE.idle}`}>{p.validate() ?? "ready"}</span>}>
