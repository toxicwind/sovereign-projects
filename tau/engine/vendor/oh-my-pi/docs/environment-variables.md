...
3. Active agent `.env` (normally `~/.tau/agent/.env`) for keys whose current value is empty/unset
4. Active config-root `.env` (normally `~/.tau/.env`) for keys whose current value is empty/unset
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE`    | Path to the encrypted local broker snapshot cache                                            | Optional in broker mode                                                                                                   | Defaults to `~/.tau/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file.                                                                                                                               |
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.tau/agent/config.yml`; environment variables are fallbacks.
...
| `PI_TINY_DEVICE`             | Inference backend for local tiny models; overrides the `providers.tinyModelDevice` setting (default: CPU; ONNX providers `cpu`, `gpu`, `webgpu`, `auto`, `cuda`, `dml`, `coreml`, `wasm`, `webnn`, `webnn-gpu`, `webnn-cpu`, `webnn-npu`, or `mlx`/`metal` to run MLX weights through mlx-lm on Apple silicon)                                                                                                                                                                                                                                                                                                                                                          |
...
| `PI_CONFIG_DIR`                                     | Config root dirname under home (default `.tau`)                                                                            |
...
| `OMP_WORKTREE_DIR`                                  | Agent-managed worktrees directory override (default `~/.tau/wt`); must be absolute or `~`-relative, relative paths are ignored; wins over the `worktree.base` setting                      |
| `OMP_GITHUB_CACHE_DB`                               | Overrides the GitHub view cache database path (default `~/.tau/cache/github-cache.db`)                                                                                                     |
