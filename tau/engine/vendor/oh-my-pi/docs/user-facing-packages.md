...
- Inputs/storage: configuration comes from `python/robomp/.env` and the mounted `~/.tau/agent/models.container.yml`; GitHub webhook events feed a SQLite-backed queue. The Compose deployment persists the database, per-issue worktrees, session transcripts, and logs in the `robomp_data` volume under `/data`.
...
- Package: private `@oh-my-pi/collab-web`; production client: <https://my.tau.sh/>.
