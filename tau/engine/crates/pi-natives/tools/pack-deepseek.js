...
function unicodeToByte() {
    ...
    for (let i = 0x21; i <= 0x7e; i++)
        bs.push(i);
    for (let i = 0xa1; i <= 0xac; i++)
        bs.push(i);
    for (let i = 0xae; i <= 0xff; i++)
        bs.push(i);
    const cs = bs.slice();
    let n = 0;
    for (let b = 0; b < 256; b++) {
    ...
    for (let i = 0; i < bs.length; i++)
        m.set(String.fromCodePoint(cs[i]), bs[i]);
    return m;
}
...
if (added.length !== 1283)
    throw new Error(`expected 1283 added_tokens, got ${added.length}`);
// rank -> token bytes, asserting contiguity 0..127999.
const byRank = new Array(VOCAB_SIZE);
let entries = 0;
for (const tok in vocab) {
    const id = vocab[tok];
    if (id < 0 || id >= VOCAB_SIZE)
        throw new Error(`vocab id ${id} out of range for "${tok}"`);
    if (byRank[id] !== undefined)
        throw new Error(`duplicate rank ${id}`);
...
if (entries !== VOCAB_SIZE)
    throw new Error(`expected ${VOCAB_SIZE} vocab entries, got ${entries}`);
for (let r = 0; r < VOCAB_SIZE; r++) {
    if (byRank[r] === undefined)
        throw new Error(`rank ${r} missing — vocab not contiguous`);
}
// Non-empty byte keys must be unique or RankTable lookups are ambiguous.
const seen = new Set();
for (const bytes of byRank) {
    if (bytes.length === 0)
        continue;
    const key = Buffer.from(bytes).toString("latin1");
    if (seen.has(key))
        throw new Error(`duplicate token byte sequence: ${JSON.stringify(key)}`);
    seen.add(key);
}
...
// a whole piece — so no drift is possible.
{
    ...
    if (merges.length !== 127741)
        throw new Error(`expected 127741 merges, got ${merges.length}`);
    const reachable = new Set();
    for (const tok in vocab)
        if ([...tok].length === 1 && u2b.has(tok))
            reachable.add(tok);
    if (reachable.size !== 256)
        throw new Error(`expected 256 alphabet entries, got ${reachable.size}`);
    for (const m of merges) {
        const parts = m.split(" ");
        if (parts.length !== 2)
            throw new Error(`malformed merge: ${JSON.stringify(m)}`);
...
const parts = [new TextEncoder().encode("UTOK1\n")];
