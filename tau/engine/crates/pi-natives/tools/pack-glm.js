...
for (const t of tj.added_tokens) {
    if (t.id < EXPECTED)
        throw new Error(`added token '${t.content}' (id ${t.id}) inside base vocab`);
    if (vocab[t.content] !== undefined)
        throw new Error(`added token '${t.content}' also in model.vocab`);
}
// Decode each key to raw bytes; assert contiguity + alphabet cleanliness.
const byRank = new Array(EXPECTED);
let seen = 0;
for (const key in vocab) {
    ...
    for (const ch of key) {
        ...
        if (b === undefined)
            throw new Error(`rank ${rank}: char U+${ch.codePointAt(0).toString(16)} not in GPT-2 byte alphabet ('${key}')`);
        bytes[n++] = b;
    }
    byRank[rank] = bytes.subarray(0, n);
}
if (seen !== EXPECTED)
    throw new Error(`vocab size ${seen} != ${EXPECTED}`);
for (let i = 0; i < EXPECTED; i++)
    if (byRank[i] === undefined)
