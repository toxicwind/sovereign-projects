...
### Auth & usage
- **Auth**: Authenticates via `GROQ_API_KEY` environment variable (`packages/catalog/src/provider-models/descriptors.ts`) as `Authorization: Bearer ${GROQ_API_KEY}` (`packages/ai/src/stream.ts:843` `getEnvApiKey("groq")` + `packages/catalog/src/discovery/openai-compatible.ts` headers). Set via `${GROQ_API_KEY}` interpolation in `.env`/`settings.json`/`config.yaml` per AGENTS.md #13. BaseUrl `https://api.groq.com/openai/v1` (OpenAI-compatible). Interactive login via `groqProvider` `loginGroq` (`packages/ai/src/registry/groq.ts`) prompts `gsk_...` and validates with `POST /chat/completions` to `https://api.groq.com/openai/v1` (model `llama-3.1-8b-instant`, 15s timeout, `AbortSignal.any`).
- **Provider Registry**: Registered as `groqProvider` (`packages/ai/src/registry/groq.ts`) with `catalogDiscovery: { label: "Groq" }` (`packages/catalog/src/provider-models/descriptors.ts:245`) — live discovery via `GET https://api.groq.com/openai/v1/models` with `Authorization: Bearer ${GROQ_API_KEY}`; fallback to stencil `tool_call:true` bundled 19 models when no key (verify: `curl -s https://api.groq.com/openai/v1/models -H "Authorization: Bearer ${GROQ_API_KEY}" | jq` — live 14 vs bundled 19).
- **Priority**: Listed 19th in provider priority ordering (`packages/catalog/src/identity/priority.ts`).

### Catalog model handling
- **Host Matching**: Matched by URL marker `api.groq.com` or provider `groq` in host definitions (`packages/catalog/src/hosts.ts:56`).
- **Manager Options**: Configured via `groqModelManagerOptions` targeting `https://api.groq.com/openai/v1` (`packages/catalog/src/provider-models/openai-compat.ts:1186`).
- **Default Model**: Defaults to `llama-3.3-70b-versatile` (`packages/catalog/src/provider-models/descriptors.ts:241`) — Llama 3.3 70B Versatile, Groq flagship (previous `openai/gpt-oss-120b` retired as default).
...
### Auth & usage
- **Authentication**: Key-based auth using NVIDIA NGC Personal Keys (`AUTH_URL = "https://org.ngc.nvidia.com/setup/personal-keys"`), stored in `NVIDIA_API_KEY` (`packages/catalog/src/provider-models/descriptors.ts:365`). Login and validation are declarative KDL (`packages/catalog/src/compat/rules/auth/nvidia.kdl`) — no `packages/ai/src/registry/nvidia.ts` exists; the provider rides the OpenAI-compat transport (`openAiCompletionsDescriptor("nvidia", … "https://integrate.api.nvidia.com/v1")`, `packages/catalog/src/provider-models/openai-compat.ts:7000`).
- **Login & Validation**: The declarative login validates keys with `validate "chat-completions" … model="openai/gpt-oss-20b" optional=#true` (`packages/catalog/src/compat/rules/auth/nvidia.kdl`). `openai/gpt-oss-20b` is the sanctioned validation model: alive-fast (~3.3–3.8s, no deprecation) in live fail-fast probes (2026-09-14); the former `nvidia/llama-3.1-nemotron-70b-instruct` is 404-gated for this key. A real auth rejection (`401`/`403`) is fatal; any other validation-endpoint failure is non-fatal — the supplied key is trusted and login proceeds.
- **Usage**: Standard OpenAI Chat Completions usage metrics; no custom usage handler or quota endpoint.
- **Deprecation headers**: NOT consumed. NIM serves a `deprecation` response header (e.g. `deprecation: 2026-10-03T09:00:00Z` on `nvidia/nemotron-3-super-120b-a12b`). Headers are normalized into `ProviderResponseMetadata`, but nothing in Tau reads `deprecation` today — pre-EOL awareness is documentation-only (see below).

### Catalog model handling
- **Descriptor & Options**: Configured via `nvidiaModelManagerOptions` (`packages/catalog/src/provider-models/openai-compat.ts:1192`) and `openAiCompletionsDescriptor` (`packages/catalog/src/provider-models/openai-compat.ts:7000`).
- **Defaults**: Default context window is `131072`. Default model is `openai/gpt-oss-20b` (`packages/catalog/src/provider-models/descriptors.ts:364`).
- **Dead model IDs to avoid**: `nvidia/llama-3.1-nemotron-70b-instruct` (catalog-listed but 404-gated for this key; removed from default and validation); `qwen/qwen3-next-80b-a3b-thinking` (HTTP 410 EOL; removed from routing).
- **Pre-EOL warning**: `nvidia/nemotron-3-super-120b-a12b` serves `deprecation: 2026-10-03T09:00:00Z` and flips nondeterministically between 503 and 200 — do not use as a default.
- **Fail-fast nondeterminism (live probes, 2026-09-14, 6s ceiling)**: `nvidia/nemotron-3-ultra-550b-a55b` and `nvidia/nemotron-3.5-lightning-30b-a3b` timed out; `z-ai/glm-5.3-flash` is cold-start-slow. None are fail-fast-safe or default material.
- **Catalog Discovery**: Registered in catalog descriptors with `catalogDiscovery: { label: "NVIDIA" }` (`packages/catalog/src/provider-models/descriptors.ts:367`).
...
## opencode-zen / opencode-go — free-tier daily-quota retry-after-ms
On the free / metered tier of both `opencode-zen` (`https://opencode.ai/zen`) and `opencode-go` (`https://opencode.ai/zen/go`), daily-quota reset responses return `HTTP 429` with `retry-after-ms=21431000` (≈5h 57m). The shared retry layer (`fetchWithRetry.maxDelayMs` = 60s and `retry.maxDelayMs` = 5 min in `packages/ai/src/utils/fetch-with-retry.ts` and `packages/ai/src/utils/retry.ts`) caps each sleep at the configured ceiling, so the hint exceeds both caps and surfaces as a hard error rather than a quiet wait-until-quota-resets retry.

Fix: add `retry: { unboundedRetryAfter: true }` to the `ProviderDefinition` (`packages/ai/src/registry/opencode-zen.ts`, `packages/ai/src/registry/opencode-go.ts`) — one line per provider file — which lifts both caps for `Retry-After`/`retry-after-ms` hints. The user setting `providers.unboundedRetryAfter` (string array, default `["opencode-go", "opencode-zen"]`) can also flip the behavior at runtime without touching provider files. See [docs/non-compaction-retry-policy.md#per-provider-unbounded-retry-after](./non-compaction-retry-policy.md#per-provider-unbounded-retry-after) for the full plumbing.
