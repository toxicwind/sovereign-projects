...
OMP discovers user agents from `~/.tau/agent/agents/*.md` and project agents from `.tau/agents/*.md`.
...
Set the role mapping in `~/.tau/agent/config.yml`:
...
- Project `.tau` overrides user `.tau`.
