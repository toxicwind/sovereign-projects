...
export class LiveProgress {
    ...
    #outputTokens = [];
    #totalTokens = [];
    #oneShotSuccessTokens = [];
    #lastLineLength = 0;
    constructor(totalRuns, runsPerTask) {
    ...
    handleEvent(event) {
        ...
        if (result && !result.success && result.error) {
            this.#flushLine();
            const header = this.#paint(ANSI.red, `[${event.taskId}] Run ${event.runIndex + 1}/${this.#runsPerTask} failed:`);
        ...
        if (result?.editFailures && result.editFailures.length > 0) {
            ...
            for (const [i, failure] of result.editFailures.entries()) {
                ...
                const metaParts = [op, target].filter((value) => Boolean(value));
    ...
    #printSummary() {
        ...
        const avgIndent = this.#indentScores.length > 0 ? this.#indentScores.reduce((a, b) => a + b, 0) / this.#indentScores.length : 0;
        this.#log("");
        this.#log(this.#paint(ANSI.bold, "Runtime Stats:"));
        this.#log(`  Task success:     ${this.#paint(rateColor(successRate), `${successRate.toFixed(1)}% (${this.#success}/${n})`)}`);
        this.#log(`  Edit success:     ${this.#paint(rateColor(editSuccessRate), `${editSuccessRate.toFixed(1)}% (${this.#totalEditSuccesses}/${this.#totalEdits})`)}`);
        ...
        const fmtTokens = (samples) => {
            if (samples.length === 0)
                return "mean=0 median=0 p1=0 p99=0";
