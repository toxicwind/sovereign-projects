...
export const EXTENDED_THINKING_LEVELS = ["minimal", "low", "medium", "high", "xhigh", "max"] as const;
export type ModelThinkingLevel = (typeof EXTENDED_THINKING_LEVELS)[number] | "off";

/** Report the thinking levels supported by a model. */
export function getSupportedThinkingLevels<TApi extends Api>(model?: Model<TApi> | null): ModelThinkingLevel[] {
	if (!model || !model.reasoning) return ["off"];
	const levels: ModelThinkingLevel[] = ["off"];
	for (const level of EXTENDED_THINKING_LEVELS) {
		if (clampThinkingLevelForModel(model, level as unknown as Effort) !== undefined) {
			levels.push(level);
		}
	}
	return levels;
