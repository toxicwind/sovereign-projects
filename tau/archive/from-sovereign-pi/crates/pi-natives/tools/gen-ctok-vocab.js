 ...
 */
function parseMarked(publicKey) {
    let out = "";
    let bytes = [];
    const flush = () => {
        if (bytes.length === 0)
...
async function generate(src, dst) {
    ...
    const out = [0x43, 0x54, 0x4f, 0x4b, 2]; // "CTOK", version
    ...
    for (const piece of sorted) {
        ...
        while (shared < max && prev[shared] === piece[shared])
            shared++;
        pushVarint(out, shared);
        pushVarint(out, piece.length - shared);
        for (let i = shared; i < piece.length; i++)
            out.push(piece[i]);
        prev = piece;
    }
