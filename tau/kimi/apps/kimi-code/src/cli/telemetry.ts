...
export function initializeCliTelemetry(options: InitializeCliTelemetryOptions): void {
  initializeTelemetry({
    ...
    // SOVEREIGN PATCH: telemetry is opt-in (`telemetry = true`), not opt-out.
...
function readServerTelemetryConfig(
  ...
  try {
    ...
    // telemetry just degrade to "disabled, no model" (SOVEREIGN PATCH:
    // opt-in, upstream degraded to "enabled") so we never block startup.
