...
/** Why a models.dev import cannot proceed at all. */
...
/**
 * The outcome of resolving a models.dev provider entry for import — the
 * single decision point for "which wire, which endpoint, or exactly why
 * not". Pattern-match on {@link ModelsDevImportResolution.kind}:
 ...
 *  - `needs-base-url`: the directory supplies no usable endpoint and the
...
/**
 * Resolves a models.dev provider entry into an import decision.
 *
 ...
 *
 * Endpoint: a user-supplied URL wins over the directory's `api` (after trim;
 * blank and `${VAR}` placeholders are rejected). Without one, the directory
...
/**
 * The wire part of {@link resolveModelsDevImport}, also used when listing
 ...
 * appends `/v1/messages` itself — so a directory `api` ending in `/v1` would
 * POST to `/v1/v1/messages` (404). Strip the trailing `/v1` for anthropic.
 * OpenAI family SDKs append `/chat/completions` to a `/v1` base, so those pass
 * through. URLs containing `${VAR}` are SDK-side env interpolations the config
 * cannot express; they resolve to `undefined` so callers can ask for a URL
 * instead.
 */
 ...
 * directory-declared and user-supplied URLs alike.
 */
...
/**
 * True when a missing directory endpoint cannot fall back to a built-in
...
/** Normalizes one models.dev model entry into a {@link ModelsDevModel}; skips invalid entries. */
 ...
 * disabling thinking (e.g. xai grok) and becomes {@link ModelsDevModel.offEffort};
