...
// SOVEREIGN PATCH: the registry default is now ON, so "flag off" tests pin it
// off explicitly via a config override.
...
describe('disclosure mode — compaction', () => {
  ...
  it('survives a runtime tool-select flag flip without a builtin refresh', async () => {
    ...
    // SOVEREIGN PATCH: registry default is now ON, so "off" must be pinned.
