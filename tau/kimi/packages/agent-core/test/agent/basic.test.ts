...
it('creates an independent agent with a scoped experimental flag resolver', () => {
  ...
  // Only `tool-select` is registered; the scoped resolver reports the
  // registry defaults (SOVEREIGN PATCH: tool-select defaults to on).
  expect(ctx.agent.experimentalFlags.enabledIds()).toEqual(['tool-select']);
});
