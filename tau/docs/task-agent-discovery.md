...
1. Nearest project `.tau/agents` dir from `findAllNearestProjectConfigDirs("agents", cwd)` (first `.omp` hit only)
2. User `.tau/agents` dir from `getConfigDirs("agents", { project: false })` (first `.omp` hit only)
