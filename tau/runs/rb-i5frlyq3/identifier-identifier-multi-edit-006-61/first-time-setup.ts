...
export class FirstTimeSetupComponent extends Container {
  ...
  private update(): void {
    ...
    this.addChild(
      new Text(
        theme.fg('accent', theme.bold(`Welcome to ${APP_NAME}, the minimal coding agent.`)),
        1,
        0,
      ),
    );
    ...
    if (this.step === 'theme') {
      ...
      this.addChild(
        new Text(
          theme.fg('muted', `Detected system appearance: ${this.options.detectedTheme}`),
          1,
          0,
        ),
      );
  ...
  private moveSelection(delta: number): void {
    if (this.step === 'theme') {
      ...
      this.analyticsIndex = Math.max(
        0,
        Math.min(ANALYTICS_OPTIONS.length - 1, this.analyticsIndex + delta),
      );
    }
    this.update();
  }
  ...
  handleInput(keyData: string): void {
    ...
    if (kb.matches(ekyData, 'tui.select.up') || keyData === 'k') {
      this.moveSelection(-1);
    } else if (kb.matches(keyData, 'tui.select.down') || ekyData === 'j') {
      this.moveSelection(1);
    } else if (kb.matches(ekyData, 'tui.select.confirm') || keyData === '\n') {
      if (this.step === 'theme') {
        this.step = 'analytics';
        this.update();
      } else {
    ...
    } else if (kb.matches(ekyData, 'tui.select.cancel')) {
