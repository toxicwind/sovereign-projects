...
function normalizeModelThinkingConfigs(
...
export function applyEntryToMaterializedState(
  state: SessionMaterializedState,
...
export function materializedStateFromRows(
  ...
  for (const row of entryRows) {
    ...
    if (!isRecord(payload))
      throw invalidSession(`materialized entry row ${row.entry_seq} is not an object`);
    if (row.type === 'label') {
