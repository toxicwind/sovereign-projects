...
| **`llama-swap`**      | `/home/toxic/projects/llama-swap`                 | `mostlygeek/llama-swap`                | Go (1.24+)       | Go AST Matrix router (`internal/flock/`), LIFO macro resolution, `normalize_sse` streaming adapter, `fuser -k` stale port cleanup, IPv4 defaults.                                   |
...
2. **`llama-swap`**: Macro hardening committed (`fix(config): harden Env slice macro validation`). All `internal/config/` and `internal/flock/` tests pass.
