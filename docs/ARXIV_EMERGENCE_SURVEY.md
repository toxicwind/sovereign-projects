...
- **NVIDIA provider**: `NVIDIA_API_KEY` (configured via env / vault) → endpoint `https://integrate.api.nvidia.com/v1` (NIM catalog / model discovery).
- **OpenRouter provider**: `OPENROUTER_API_KEY` (configured via env / vault) → model `openrouter/thinkingmachines/inkling:free` (fast tier), `high` reasoning tier supported.
- **Herd (local)**: model `beellama/qwen-flash-64k` running locally on port `25100` (RTX 3090, 64K context, 119 tok/s, CUDA 86). Self-hosted, no external key.
- **Hindsight API**: port `25117` (self-hosted, health endpoint `/health`, verified by registry health-guard). Independent from Herd; both must respond healthy for `registry.emergent_sync` activation.
