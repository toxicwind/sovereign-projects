...
| tau-hyperfix | `/home/toxic/.tau` audit; dynamic skill loading; skills symlink; `skillful`; `tau audit`; `tau tmux` experiments | Ember | DONE (2026-09-20) -- audit 13 pass/0 fail, collapse 8/8, tmux live, `tau launch -p` -> ALIVE; deliverables (launcher/tau + audit/tmux helpers + install.sh) live ONLY as local yote commit `a356d831ee06` ("tau: canonical launcher promotion", tau-closer) which is NOT on origin/main — verified 2026-09-21 via merge-base; earlier KB citation implied a canonical SHA; re-verify + push the launcher work before citing a remote SHA |
...
| yote-consolidation (ember-ironwright) | Herd FQN resolution + config/binary durability + Telegram/OpenFang delivery reliability (Chris: "Do all please when it's back") | ember-ironwright (Ember's crew) | WS1 DONE (2026-09-21): FQN regression test + source to toxicwind/sovereign-swap main 61e497ee; content-hash-gated immutable deploy live :25100 (bin/herd-deploy, bin/herd-config-validate); bare+FQN 200, incident FQN 200, openfang:coyote 200. WS2 durability + WS3 telegram RUNNING |
