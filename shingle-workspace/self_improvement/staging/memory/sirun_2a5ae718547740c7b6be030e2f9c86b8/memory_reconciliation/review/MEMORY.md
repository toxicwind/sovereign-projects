...
## Preferences
- Local chat build is the first-class priority — first in the queue, fully unblocked and building; second paper-screening pass cancelled, merge proceeds with the 12 papers already selected.
...
- Mirror audit (11 TLDs, curl_cffi + Camoufox): `.is` = only mirror serving real book pages (dl buttons all -> /account); official `.gl`/`.gd`/`.pk` serve homepage but 404 on ALL /books/ URLs (genuinely broken, not bot filtering); `.li` = CHEQ bot-check; `.gs` slow but alive; `.pm`/`.vg` drop connections; `.in`/`.org`/`.se` NXDOMAIN.
...
- History purged of committed `.env` (held GitHub PAT + other API keys): all 60 main commits rebuilt via git API (new head `508fd0c4`); zero commits touch `.env`; `.gitignore` already listed it (was force-added).
...
- Tau NVIDIA provider hardening COMPLETE (verified): main==origin/main at 8b0d4c79, tree clean; 40ff7acb (removed stray kimi token files), 924f7c40 (drop EOL qwen3-next, dedupe glm-5.3-flash), 8b0d4c79 (default gpt-oss-20b, drop stale nemotron-effort test, refresh quirks, new wire-compat test); sanctioned fast-forward, temp branch deleted.
