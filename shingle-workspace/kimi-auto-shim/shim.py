...
kimi-auto routes Kimi models ONLY. There is deliberately no non-Kimi
fallback: when no Kimi candidate is healthy the shim fails loudly (503)
instead of silently serving a different vendor's model.
...
NO_KIMI_MSG = (
    "kimi-auto: no healthy Kimi model available ({reason}). "
    "Provide KIMI_API_KEY or MOONSHOT_API_KEY so the resolver can select one."
)


def load_state(state_path):
    """Return {model, updated_at, healthy, reason}. Never raises."""
    try:
        ...
        return {
            ...
            "updated_at": data.get("updated_at"),
            "healthy": False,
            "reason": "state has no model (" + reason + ")",
        }
        ...
        return {
            ...
            "updated_at": None,
            "healthy": False,
            "reason": "state load failed: %s" % exc,
        }


def resolve_target(state):
    """Return (model_id, error). error is None when routable."""
    if not state["healthy"] or not state["model"]:
        return None, NO_KIMI_MSG.format(reason=state["reason"])
    if state["model"].lower() == "kimi-auto":
        # Self-reference guard: never route the alias back into itself.
        return None, NO_KIMI_MSG.format(
            reason="resolver state points at the kimi-auto alias itself"
        )
...
class Handler(BaseHTTPRequestHandler):
            ...
            self._send_json(200, {
                ...
                "data": [
                    {"id": "kimi-auto", "object": "model", "owned_by": "kimi-auto",
                     "metadata": {"resolves_to": state["model"],
                                 "healthy": state["healthy"]}},
                ],
            })
        else:
    ...
    def do_POST(self):
        ...
        if requested in ("kimi-auto", None, ""):
            state = load_state(self.server.state_path)
            target, error = resolve_target(state)
            if error:
                print("[kimi-auto] refusing request: %s" % error, flush=True)
        ...
        try:
            ...
            self._send_json(503, {"error": "herd unreachable: %s" % exc})
            return
...
def main():
    ...
    print("[kimi-auto] shim on 127.0.0.1:%d -> herd %s" % (args.port, args.herd), flush=True)
    print("[kimi-auto] state: %s (Kimi-only, no non-Kimi fallback)" % args.state, flush=True)
    try:
