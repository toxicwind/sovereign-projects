...
5. io-governor self-heal: if `pgrep -f "[b]in/io-governor"` finds nothing, restart it as root: `sudo -n setsid nohup python3 /home/hatch/workspace/bin/io-governor >> /home/hatch/workspace/psi-monitor/governor.log 2>&1 < /dev/null &`. The daemon is the reactive I/O-stall triage (never kills; leased work paged only, unleased heavy writers ionice-idled + paged). If sudo fails, report WATCH-FAIL with the sudo error — do not run it unprivileged (ionice on other PIDs needs root). (NOTE: the [b] bracket trick is load-bearing — a bare pattern matches the worker shell's own command line and false-positives as alive)
