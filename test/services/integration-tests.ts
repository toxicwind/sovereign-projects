...
export async function testYoteHealth(): Promise<ChainResult> {
  ...
  try {
    ...
    return {
      ...
      error: passed ? undefined : `HTTP ${res.status}: ${body}`,
    };
  } catch (e) {
    return {
      name: "yote-health",
...
export async function testYoteRoot(): Promise<ChainResult> {
  ...
  try {
    ...
    return {
      ...
      details: {
        svc: body.svc,
        overlord: body.overlord,
        openfang_url: body.openfang_url,
        bot_token_set: body.bot_token_set,
      },
      error: passed
        ? undefined
        : `Unexpected: ${JSON.stringify(body).slice(0, 200)}`,
    };
  } catch (e) {
    return {
      name: "yote-root",
...
export async function testOverlordConnected(): Promise<ChainResult> {
  ...
  try {
    ...
    return {
      name: "overlord-connected",
      passed,
      latencyMs: Date.now() - start,
      details: { overlord: body.overlord, bot_token_set: body.bot_token_set },
      error: passed
        ? undefined
        : `Overlord not connected: overlord=${body.overlord}, bot=${body.bot_token_set}`,
    };
  } catch (e) {
    return {
      name: "overlord-connected",
...
export async function testOpenFangHealth(): Promise<ChainResult> {
  const start = Date.now();
  try {
    const res = await fetch(`${OF_URL}/api/health`, {
    ...
    return {
      ...
      passed,
      latencyMs: Date.now() - start,
      details: { status: res.status, bodyLength: body.length },
      error: passed ? undefined : `OpenFang health failed: HTTP ${res.status}`,
    };
  } catch (e) {
    return {
      name: "openfang-health",
...
export async function testHerdHealth(): Promise<ChainResult> {
  ...
  try {
    ...
    return {
      ...
      error: passed ? undefined : `Herd health failed: HTTP ${res.status}`,
    };
  } catch (e) {
    return {
      name: "herd-health",
...
export async function testNexusMcpHealth(): Promise<ChainResult> {
  ...
  try {
    ...
    const passed = res.ok;
    return {
      name: "nexus-mcp-health",
      passed,
      latencyMs: Date.now() - start,
      details: { status: res.status },
      error: passed ? undefined : `Nexus MCP health failed: HTTP ${res.status}`,
    };
  } catch (e) {
    return {
      name: "nexus-mcp-health",
...
export async function testKimiCodeWebUi(): Promise<ChainResult> {
  ...
  try {
    ...
    return {
      name: "kimi-code-web-ui",
      passed,
      latencyMs: Date.now() - start,
      details: {
        status: res.status,
        has_kimi_code: body.includes("Kimi Code"),
      },
      error: passed ? undefined : `Kimi Code UI failed: HTTP ${res.status}`,
    };
  } catch (e) {
    return {
      name: "kimi-code-web-ui",
...
export async function testYoteOpenFangDispatch(): Promise<ChainResult> {
  ...
  try {
    const res = await fetch(`${YOTE_URL}/api/openfang/chat`, {
      ...
      body: JSON.stringify({
        message: "Reply with: YOTE_CHAIN_OK",
        agent: "coyote",
        max_tokens: 64,
      }),
    ...
    return {
      ...
      error: passed
        ? undefined
        : `Dispatch: ${JSON.stringify(body).slice(0, 200)}`,
    };
  } catch (e) {
    return {
      name: "yote-openfang-dispatch",
...
export async function testYoteHerdFallback(): Promise<ChainResult> {
  ...
  try {
    const res = await fetch(`${YOTE_URL}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        model: "llama",
        messages: [{ role: "user", content: "Reply with: YOTE_HERD_OK" }],
        max_tokens: 32,
      }),
      signal: AbortSignal.timeout(30000),
    });
    ...
    return {
      name: "yote-herd-fallback",
      passed,
      latencyMs: Date.now() - start,
      details: {
        status: res.status,
        model: body.model,
        has_choices: !!body.choices,
        has_error: !!body.error,
      },
      error: passed
        ? undefined
        : `Herd fallback: ${JSON.stringify(body).slice(0, 200)}`,
    };
  } catch (e) {
    return {
      name: "yote-herd-fallback",
  ...
  try {
    ...
    if (!fs.existsSync(bridgePath))
      return {
        ...
        passed: false,
        latencyMs: Date.now() - start,
        error: `Bridge not found at ${bridgePath}`,
      };
    const content = fs.readFileSync(bridgePath, "utf8");
    const hasOpenFang =
      content.includes("OPENFANG_URL") || content.includes("25103");
    ...
    return {
      ...
      passed,
      latencyMs: Date.now() - start,
      details: { path: bridgePath, hasOpenFang, hasHerd, hasHal },
      error: passed ? undefined : "Bridge missing endpoint references",
    };
  } catch (e) {
    return {
      name: "kimi-claw-bridge",
...
export async function testOverlordAdminVerify(): Promise<ChainResult> {
  ...
  try {
    ...
    const passed =
      body.overlord === true &&
      body.openfang_url != null &&
      body.bot_token_set === true;
    return {
      name: "overlord-admin-verify",
      passed,
      latencyMs: Date.now() - start,
      details: { ...body },
      error: passed
        ? undefined
        : `Admin verify failed: ${JSON.stringify(body).slice(0, 200)}`,
    };
  } catch (e) {
    return {
      name: "overlord-admin-verify",
...
export async function runIntegrationTests(): Promise<ChainResult[]> {
  ...
  for (const fn of tests) {
    ...
    console.log(
      `${status} ${r.name} (${r.latencyMs}ms)${r.error ? " — " + r.error : ""}`,
    );
  }
