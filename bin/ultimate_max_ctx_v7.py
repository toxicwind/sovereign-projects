#!/usr/bin/env python3
"""
Ultimate Max Context Finder v7 – Final
- Capped exponential backoff polling (max 2s delay)
- Dynamic type discovery from ggml.h
- Binary search per working type
- Environment variable support
- Live JSON events + final report
...
MODEL_PATH = pathlib.Path(get_env_or_default(
    "MODEL_PATH",
    "/home/toxic/models/Qwen3.6-27B-Heretic-UD/heretic-UD-27B-Q5_K_XL.gguf"
))
MMPROJ_PATH = pathlib.Path(get_env_or_default(
    "MMPROJ_PATH",
    str(MODEL_PATH.parent / "mmproj-27B-F16.gguf")
))
SERVER_BIN = pathlib.Path(get_env_or_default(
    "SERVER_BIN",
    "/home/toxic/ik_llama.cpp-main/build/bin/llama-server"
))
BENCH_BIN = pathlib.Path(get_env_or_default(
    "BENCH_BIN",
    "/home/toxic/ik_llama.cpp-main/build/bin/llama-bench"
))
GGML_H_PATH = pathlib.Path(get_env_or_default(
    "GGML_H_PATH",
    "/home/toxic/ik_llama.cpp-main/ggml/include/ggml.h"
))
PORT_BASE = int(os.environ.get("PORT_BASE", "28080"))
TIMEOUT_GEN = int(os.environ.get("TIMEOUT_GEN", "60"))

# ============================================================================
# DYNAMIC TYPE DISCOVERY (filtered)
# ============================================================================
...
def poll_server_ready(port: int, max_attempts: int = 10, base_delay: float = 0.2, max_delay: float = 2.0) -> bool:
    """Poll /v1/models until it responds, with capped exponential backoff."""
    import requests
...
def test_ctx(ctx: int, cache_type: str) -> Dict:
    ...
    # Poll for readiness
    ready = poll_server_ready(port, max_attempts=10, base_delay=0.2, max_delay=2.0)
...
def test_type_quick(cache_type: str) -> Tuple[bool, str]:
    ...
    try:
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        ready = poll_server_ready(PORT_BASE, max_attempts=10, base_delay=0.2, max_delay=2.0)
        if ready:
            kill_all()
            return True, ""
        else:
...
def main():
    # Ensure requests is available (uv run --with requests)
    try:
        import requests
    except ImportError:
        print("requests not found; run with: uv run --with requests --script ...", file=sys.stderr)
        sys.exit(1)
    ...
    # Quick discovery with progress
    working_types = []
    total = len(all_types)
    for idx, (ct, bits) in enumerate(all_types, 1):
        sys.stdout.write(json.dumps({"event": "discovery", "cache_type": ct, "status": "testing", "progress": f"{idx}/{total}"}) + "\n")
        sys.stdout.flush()
        ok, err = test_type_quick(ct)
        if ok:
            working_types.append((ct, bits))
            sys.stdout.write(json.dumps({"event": "discovery", "cache_type": ct, "status": "works", "progress": f"{idx}/{total}"}) + "\n")
        else:
            sys.stdout.write(json.dumps({"event": "discovery", "cache_type": ct, "status": "fails", "error": err, "progress": f"{idx}/{total}"}) + "\n")
        sys.stdout.flush()
    ...
    out_file = pathlib.Path(os.environ.get("REPORT_PATH", "/home/toxic/sovereign/canonical_benchmark_v7.json"))
