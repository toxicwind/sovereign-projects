...
const HTTP_SERVICES: ServiceEndpoint[] = [
  ...
  { name: "axiom/openfang", url: "http://127.0.0.1:25103/api/health", port: 25103 },
  { name: "prometheus", url: "http://127.0.0.1:25105/-/healthy", port: 25105 },
  { name: "hf-downloader", url: "http://127.0.0.1:25106/api/health", port: 25106 },
  ...
  { name: "mesh/mcpproxy-go", url: "http://127.0.0.1:25127/health", port: 25127 },
...
async function checkTcp(endpoint: TcpEndpoint): Promise<ProbeResult> {
  ...
  return new Promise<ProbeResult>((resolve) => {
    const socket = createConnection({ host: "127.0.0.1", port: endpoint.port, timeout: 2000 }, () => {
      const elapsed_ms = Number((performance.now() - t0).toFixed(1));
      socket.destroy();
      resolve({
        service: endpoint.name,
...
async function main() {
  ...
  for (const r of allResults) {
    ...
    const payload = r.response !== undefined ? JSON.stringify(r.response) : r.error;
    console.log(`${icon} ${r.service.padEnd(20)} [:${r.port}] ${r.elapsed_ms.toString().padStart(5)}ms -> ${payload}`);
  }
