...
const TOOLS = [
  ...
  {
    name: "tmux_capture",
    description: "Capture pane scrollback",
    inputSchema: {
      type: "object",
  ...
  {
    name: "tmux_send",
    description: "Send keys/command to pane",
    inputSchema: {
      type: "object",
  ...
  while (true) {
    ...
    if (msg.method === "initialize") {
      writeRpc({
        ...
        result: {
          capabilities: { tools: {} },
          serverInfo: { name: "tmux-mcp", version: "1.0" },
        },
      });
      ...
      if (msg.params.name === "tmux_list") {
        ...
        writeRpc({
          jsonrpc: "2.0",
          id: msg.id,
          result: { content: [{ type: "text", text: out || "No sessions" }] },
        });
