...
4. **Resolution:** To prevent this, the root `pyproject.toml` lists `qwen35-megakernel-bf16` in `no-build-isolation-package`. Thus, the extension must be compiled against the *existing* host virtual environment, necessitating a two-pass installation flow:
...
2. **Verify Megakernel compiled library is importable and correct:**
   ...
   ```
   *(This script tests the correctness of the megakernel output against a reference decode path).*
