...
_Note: In the same directory, there is an even more comprehensive script, `bin/ultimate_max_ctx_v8.py`, which performs binary searches across three KV cache quantization settings (`q4_0`, `q8_0`, `f16`), queries the system VRAM dynamically using `nvidia-smi`, and saves results in JSON format._
...
_Verification Command:_ `ls -la /home/toxic/models/Qwen3.6-27B-Heretic-UD/`
...
_(Alternatively, execute `python3 bin/ultimate_max_ctx_v8.py` to test q4_0 KV cache limits and obtain a detailed JSON metric report)._
